#!/bin/bash
# Copyright (c) 2013 The Zenacoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Zenacoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Zenacoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Zenacoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Zenacoin 0.7 datadir."
    ;;
  3)
    echo "Detected Zenacoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Zenacoin 0.8 datadir."
    ;;
esac
