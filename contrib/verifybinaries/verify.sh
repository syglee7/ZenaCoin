...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'zenacoin-',
...
if [ $? -ne 0 ]; then
   echo "zenacoin.org failed to provide signature file, but zenacoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "zenacoin.org and zenacoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Zenacoin Core binary release signing key installed?"
   fi
