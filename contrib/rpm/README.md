...
The RPM spec file provided here is for Zenacoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://zenacoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Zenacoin git repository](https://github.com/zenacoin/zenacoin) but are not part of
...
Currently only `Source10` is used, for the example `zenacoin.conf` file.
...
in the zenacoin git repository. At present only an SVG version of the Zenacoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the zenacoin
...
needed when 0.12.1 is released, a proper fix is already in the Zenacoin git
master branch.
...
idea to build Zenacoin Core against a static version of that library compiled
...
The `zenacoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `zenacoin-server` package. At this point, most RPM based
...
The `zenacoin-server` package should have SELinux support. How to properly do
...
Zenacoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/zenacoin/). He has been
packaging Zenacoin for Fedora at least since 2012.
...
4. I renamed the `zenacoin` package that contains the Qt GUI to `zenacoin-core` as
that appears to be how the general population refers to it, in contrast to
`zenacoin-xt` or `zenacoin-classic`. I wanted to make sure the general population
