...
*Setup instructions for a Gitian build of Zenacoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Zenacoin
...
- [Building Zenacoin Core](#building-zenacoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-zenacoin-host/README.md).
...
Clone the git repositories for zenacoin and Gitian.
...
in the zenacoin repository under 'Fetch and create inputs' to install sources which require
...
To build Zenacoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the zenacoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/zenacoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/zenacoin/zenacoin
...
For example:
```bash
URL=https://github.com/laanwj/zenacoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit zenacoin=${COMMIT} --url zenacoin=${URL} ../zenacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit zenacoin=${COMMIT} --url zenacoin=${URL} ../zenacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit zenacoin=${COMMIT} --url zenacoin=${URL} ../zenacoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the zenacoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../zenacoin/contrib/gitian-descriptors/*|sort|uniq )
...
ZNCPATH=/some/root/path/zenacoin
SIGPATH=/some/root/path/zenacoin-detached-sigs

./bin/gbuild --url zenacoin=${ZNCPATH},signature=${SIGPATH} ../zenacoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/zenacoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/zenacoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/zenacoin-osx-build.assert
```
...
[zenacoin-core/gitian.sigs](https://github.com/zenacoin-core/gitian.sigs/) repository, or if that's not possible create a pull
