...
const char* BIP71_MIMETYPE_PAYMENT = "application/zenacoin-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/zenacoin-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/zenacoin-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(ZENACOIN_IPC_PREFIX, Qt::CaseInsensitive)) // zenacoin: URI
        {
            ...
            {
                CZenacoinAddress address(r.address.toStdString());
...
//
// OSX-specific way of handling zenacoin: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in zenacoin: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(ZENACOIN_IPC_PREFIX, Qt::CaseInsensitive)) // zenacoin: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            {
                CZenacoinAddress address(recipient.address.toStdString());
                if (!address.IsValid()) {
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CZenacoinAddress(dest).ToString()));
        }
        else if (!recipient.authenticatedMerchant.isEmpty()) {
            // Unauthenticated payment requests to custom zenacoin addresses are not supported
        ...
        // Zenacoin amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (IsDust(txOut, ::dustRelayFee)) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(ZenacoinUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
