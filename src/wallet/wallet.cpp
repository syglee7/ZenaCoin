...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CZenacoinAddress(CScriptID(redeemScript)).ToString();
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/zenacoin/zenacoin/pull/10340, but that change has been
    // postponed until after https://github.com/zenacoin/zenacoin/pull/10244 to
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(CZenacoinAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(CZenacoinAddress(address).ToString(), strName);
}
...
{
    {
        ...
        std::string strAddress = CZenacoinAddress(address).ToString();
    ...
    CWalletDB(*dbw).ErasePurpose(CZenacoinAddress(address).ToString());
    return CWalletDB(*dbw).EraseName(CZenacoinAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(CZenacoinAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(CZenacoinAddress(dest).ToString(), key);
}
